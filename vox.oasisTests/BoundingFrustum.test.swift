...
class BoundingFrustumTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        vpMatrix = Matrix()
        Matrix.multiply(left: projectionMatrix, right: viewMatrix, out: vpMatrix)
    ...
    func testIntersectsSphere() {
        ...
        BoundingSphere.fromBox(box: box1, out: sphere1)
        let flag1 = frustum.intersectsSphere(sphere: sphere1)
        XCTAssertEqual(flag1, true)

        let box2 = BoundingBox(Vector3(-32, -2, -2), Vector3(-28, 2, 2))
        let sphere2 = BoundingSphere()
        BoundingSphere.fromBox(box: box2, out: sphere2)
