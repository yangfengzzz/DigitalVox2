...
class AdditiveAnimationBuilderTests: XCTestCase {
    ...
    func testBuildRefPose() {
        ...
        do {
            var ref_pose = [VecTransform.identity(), VecTransform.identity(), VecTransform.identity()]
            ref_pose[0] = VecTransform.identity()
            ref_pose[1].translation = VecFloat3(1.0, 1.0, 1.0)
            ref_pose[1].rotation = VecQuaternion(0.0, 0.0, 0.70710677, 0.70710677)
            ref_pose[1].scale = VecFloat3(1.0, -1.0, 2.0)
            ...
            var output = RawAnimation()
            XCTAssertTrue(builder.eval(input, ref_pose[...], &output))
            XCTAssertEqual(output.num_tracks(), 3)
