...
bool BuildVertices(FbxMesh *_fbx_mesh,
    ...
    // rejected.
    Mesh::Part &part = _output_mesh->parts[0];
    part.positions.reserve(vertex_count *
            Mesh::Part::kPositionsCpnts);
    part.normals.reserve(vertex_count * Mesh::Part::kNormalsCpnts);
    if (element_tangents) {
        part.tangents.reserve(vertex_count *
                Mesh::Part::kTangentsCpnts);
    }
    if (element_uvs) {
        part.uvs.reserve(vertex_count * Mesh::Part::kUVsCpnts);
    }
    if (element_colors) {
        part.colors.reserve(vertex_count * Mesh::Part::kColorsCpnts);
    }
    ...
    for (int p = 0; p < polygon_count; ++p) {
        ...
        for (int v = 0; v < 3; ++v, ++vertex_id) {
            ...
            for (size_t r = 0; r < remap.size(); ++r) {
                ...
                if (!Compare(
                        &normal.x,
                        &part.normals[to_test * Mesh::Part::kNormalsCpnts],
                        Mesh::Part::kNormalsCpnts)) {
                ...
                if (element_uvs) {
                    if (!Compare(&uv.x,
                            &part.uvs[to_test * Mesh::Part::kUVsCpnts],
                            Mesh::Part::kUVsCpnts)) {
                        continue;  // Next vertex.
                    }
                }
                ...
                if (element_colors) {
                    if (!Compare(
                            color,
                            &part.colors[to_test * Mesh::Part::kColorsCpnts],
                            Mesh::Part::kColorsCpnts)) {
                        continue;  // Next vertex.
                    }
                }
            ...
            if (redundant_with >= 0) {
                ...
                if ((part.positions.size() / 3) >
                    ...
                    std::cerr << "Mesh uses too many vertices (> "
                              << std::numeric_limits<uint16_t>::max()
                              << ") to fit in the index "
                                 "buffer."
                              << std::endl;
                ...
                uint16_t vertex_index = static_cast<uint16_t>(
                        part.positions.size() / Mesh::Part::kPositionsCpnts);
...
bool BuildSkin(FbxMesh *_fbx_mesh,
    ...
    if (skin_count == 0) {
        std::cerr << "No skin found." << std::endl;
        return false;
    }
    if (skin_count > 1) {
        std::cout
    ...
    if (skinning_type != FbxSkin::eRigid && skinning_type != FbxSkin::eLinear) {
        std::cerr << "Unsupported skinning type" << std::endl;
        return false;
    }
    ...
    for (int cl = 0; cl < cluster_count; ++cl) {
        ...
        if (!node) {
            std::cout << "No node linked to cluster " << cluster->GetName()
                      << "." << std::endl;
            continue;
        }
        ...
        if (mode != FbxCluster::eNormalize) {
            std::cerr << "Unsupported link mode for joint " << node->GetName()
                      << "." << std::endl;
            return false;
        }
        ...
        if (it == joints_map.end()) {
            std::cerr << "Required joint " << node->GetName()
    ...
    if (vertex_isnt_influenced) {
        std::cerr << "At least one vertex isn't influenced by any joints."
                  << std::endl;
    }
...
bool LimitInfluences(Mesh &_skinned_mesh, int _limit) {
    assert(_skinned_mesh.parts.size() == 1);

    Mesh::Part &in_part = _skinned_mesh.parts.front();
...
// matrices.
bool RemapIndices(Mesh *_skinned_mesh) {
    assert(_skinned_mesh->parts.size() == 1);
    ...
    Mesh::Part::JointIndices local_indices = in_part.joint_indices;
    ...
    // are set to 0.
    Mesh::Part::JointIndices original_remap(
            _skinned_mesh->num_joints(), 0);
    for (size_t i = 0; i < local_indices.size(); ++i) {
        original_remap[local_indices[i]] =
                static_cast<Mesh::Part::JointIndices::value_type>(i);
    }
...
bool SplitParts(const Mesh &_skinned_mesh,
        Mesh *_partitionned_mesh) {
    assert(_skinned_mesh.parts.size() == 1);
    assert(_partitionned_mesh->parts.size() == 0);

    const Mesh::Part &in_part = _skinned_mesh.parts.front();
    const size_t vertex_count = in_part.vertex_count();
    ...
    for (int i = 0; i < max_influences; ++i) {
        ...
        Mesh::Part &out_part = _partitionned_mesh->parts.back();
        ...
        for (size_t j = 0; j < bucket_vertex_count; ++j) {
            ...
            // Fills positions.
            float *out_pos =
                    &out_part.positions[j * Mesh::Part::kPositionsCpnts];
            const float *in_pos =
            ...
            // Fills normals.
            float *out_normal =
                    &out_part.normals[j * Mesh::Part::kNormalsCpnts];
            ...
            // Fills uvs.
            if (in_part.uvs.size()) {
                float *out_uv = &out_part.uvs[j * Mesh::Part::kUVsCpnts];
                const float *in_uv =
                        &in_part
                                .uvs[bucket_vertex_index * Mesh::Part::kUVsCpnts];
                out_uv[0] = in_uv[0];
                out_uv[1] = in_uv[1];
            }
            // Fills colors.
            if (in_part.colors.size()) {
                uint8_t *out_color =
                        &out_part.colors[j * Mesh::Part::kColorsCpnts];
            ...
            if (in_part.tangents.size()) {
                float *out_tangent =
                        &out_part.tangents[j * Mesh::Part::kTangentsCpnts];
...
// weights equals 1).
bool StripWeights(Mesh *_mesh) {
    for (size_t i = 0; i < _mesh->parts.size(); ++i) {
        Mesh::Part &part = _mesh->parts[i];
