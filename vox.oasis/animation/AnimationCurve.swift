...
class AnimationCurve {
    ...
    private func _evaluateLinear(_ frameIndex: Int, _ nextFrameIndex: Int, _ t: Float) -> InterpolableValue {
        ...
        case .Vector2:
            let value = Vector2()
            Vector2.lerp(left: keys[frameIndex].getVector2Value(),
                    right: keys[nextFrameIndex].getVector2Value(),
                    t: t, out: value)
            _currentValue = .Vector2(value)
            return _currentValue
        case .Vector3:
            let value = Vector3()
            Vector3.lerp(left: keys[frameIndex].getVector3Value(),
                    right: keys[nextFrameIndex].getVector3Value(),
                    t: t, out: value)
            _currentValue = .Vector3(value)
            return _currentValue
        case .Quaternion:
            let value = Quaternion()
            Quaternion.slerp(start: keys[frameIndex].getQuaternionValue(),
                    end: keys[nextFrameIndex].getQuaternionValue(),
                    t: t, out: value)
            _currentValue = .Quaternion(value)
            return _currentValue
        default:
            fatalError()
        }
    ...
    private func _evaluateHermite(_ frameIndex: Int, _  nextFrameIndex: Int, _  t: Float, _  dur: Float) -> InterpolableValue {
        ...
        case 4:
            ...
            t1 = tan1.w
            if (t0.isLessThanOrEqualTo(Float.greatestFiniteMagnitude) && t1.isLessThanOrEqualTo(Float.greatestFiniteMagnitude)) {
                value.w = a * p0.w + b * t0 * dur + c * t1 * dur + d * p1.w
            } else {
                value.w = p0.w
            }
